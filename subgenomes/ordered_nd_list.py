...
class OrderedNDList(Subgenome):
    ...
    def mutate(self):
        ...
        no duplicates are inserted. The `num_swaps` is calculated with respect to `self.mutation_probability` and is
        1 or higher.
        """
        num_swaps = max(1, int(round(self.mutation_probability / 2 * len(self.items))))
